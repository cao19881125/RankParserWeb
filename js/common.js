function fill_table(data,tablename){
        ...
        for(var pd in data){
            ...
            per_data_array.push("<a class=\"text-primary\" href=\"" + trdata.url + "\">" + trdata.name + "</a>")
            //per_data_array.push(trdata.name)
            per_data_array.push(trdata.score)
            per_data_array.push(trdata.rank)
            per_data_array.push("<a class=\"text-primary\" href=\"" + trdata.serverRankHref + "\">" +  trdata.serverRank.toString() + "（" + trdata.serverRankDps.toString() + ")</a>")
